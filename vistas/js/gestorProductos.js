...
/*=============================================
ACTIVAR PRODUCTO MAS VENDIDO
=============================================*/
$(".tablaProductos tbody").on("click", ".btnActivarVendido", function () {
  var idProducto = $(this).attr("idProducto");
  var estadoProducto = $(this).attr("estadoProducto");

  var datos = new FormData();
  datos.append("activarId", idProducto);
  datos.append("activarProductoVendido", estadoProducto);
...
$(".fotoPortada").change(function () {
  ...
  if (
    imagenPortada["type"] != "image/jpeg" &&
    imagenPortada["type"] != "image/png"
  ) {
    $(".fotoPortada").val("");
...
$(".fotoPrincipal").change(function () {
  ...
  if (
    imagenFotoPrincipal["type"] != "image/jpeg" &&
    imagenFotoPrincipal["type"] != "image/png"
  ) {
    $(".fotoPrincipal").val("");
...
$("#modalCrearProducto .valorOferta").change(function () {
  if ($(".precio").val() != 0) {
    if ($(this).attr("tipo") == "oferta") {
      var descuento =
        100 - (Number($(this).val()) * 100) / Number($(".precio").val());
    ...
    if ($(this).attr("tipo") == "descuento") {
      var oferta =
        Number($(".precio").val()) -
        (Number($(this).val()) * Number($(".precio").val())) / 100;
...
$(".fotoOferta").change(function () {
  ...
  if (
    imagenOferta["type"] != "image/jpeg" &&
    imagenOferta["type"] != "image/png"
  ) {
    $(".fotoOferta").val("");
...
function agregarMiProducto(imagen) {
  ...
  var rutaProductoMercadoLibre = $(".rutaProductoMercadoLibre").val();
  ...
  datosProducto.append("rutaProductoMercadoLibre", rutaProductoMercadoLibre);
...
$(".tablaProductos tbody").on("click", ".btnEditarProducto", function () {
  ...
  $.ajax({
    ...
    success: function (respuesta) {
      ...
      $("#modalEditarProducto .rutaProductoMercadoLibre").val(
        respuesta[0]["urlBuy"]
      );
      ...
      if (respuesta[0]["multimedia"] != "") {
        ...
        for (var i = 0; i < imagenesMultimedia.length; i++) {
          $(".previsualizarImgFisico").append(
            '<div class="col-md-3">' +
              ...
              '<img class="imagenesRestantes" src="' +
              imagenesMultimedia[i].foto +
              '" style="width:100%">' +
          ...
          localStorage.setItem(
            "multimediaFisica",
            JSON.stringify(imagenesMultimedia)
          );
        }
        ...
        $(".removerImagen").click(function () {
          ...
          localStorage.setItem(
            "multimediaFisica",
            JSON.stringify(arrayImgRestantes)
          );
        });
      }
      ...
      let marca = document.getElementById("detalleMarca");
      marca.value = detalles.Marca;

      let codigo = document.getElementById("codigoProducto");
      codigo.value = detalles.Codigo;

      /*=============================================
			TRAEMOS LA CATEGORIA
			=============================================*/

      if (respuesta[0]["categoryId"] != 0) {
        var datosCategoria = new FormData();
        datosCategoria.append("idCategoria", respuesta[0]["categoryId"]);
        ...
        $.ajax({
          ...
          success: function (respuesta) {
            ...
            $("#modalEditarProducto .optionEditarCategoria").html(
              respuesta["category"]
            );
          },
        });
      ...
      if (respuesta[0]["subcategoryId"] != 0) {
        var datosSubCategoria = new FormData();
        datosSubCategoria.append(
          "idSubCategoria",
          respuesta[0]["subcategoryId"]
        );
        ...
        $.ajax({
          ...
          dataType: "json",
          success: function (respuesta) {
            $("#modalEditarProducto .optionEditarSubCategoria").val(
              respuesta[0]["id"]
            );
            $("#modalEditarProducto .optionEditarSubCategoria").html(
              respuesta[0]["subcategory"]
            );

            var datosCategoria = new FormData();
            datosCategoria.append("idCategoria", respuesta[0]["catgeoryId"]);
      ...
      datosCabecera.append("ruta", respuesta[0]["route"]);
      ...
      $.ajax({
        ...
        success: function (respuesta) {
          ...
          if (respuesta["keywords"] != null) {
            $("#modalEditarProducto .editarPalabrasClaves").html(
              '<div class="input-group">' +
                ...
                '<input type="text" class="form-control input-lg tagsInput pClavesProducto" value="' +
                respuesta["keywords"] +
                '" data-role="tagsinput">' +
                "</div>"
            );
          ...
          $("#modalEditarProducto .previsualizarPortada").attr(
            "src",
            respuesta["frontPage"]
          );
          $("#modalEditarProducto .antiguaFotoPortada").val(
            respuesta["frontPage"]
          );
        },
      });
      ...
      $("#modalEditarProducto .previsualizarPrincipal").attr(
        "src",
        respuesta[0]["frontPage"]
      );
      $("#modalEditarProducto .antiguaFotoPrincipal").val(
        respuesta[0]["frontPage"]
      );
      ...
      if (respuesta[0]["offer"] != 0) {
        ...
        $("#modalEditarProducto .precioOferta").val(respuesta[0]["offerPrice"]);
        $("#modalEditarProducto .descuentoOferta").val(
          respuesta[0]["discountOffer"]
        );
        ...
        $("#modalEditarProducto .previsualizarOferta").attr(
          "src",
          respuesta[0]["imgOffer"]
        );

        $("#modalEditarProducto .antiguaFotoOferta").val(
          respuesta[0]["imgOffer"]
        );

        $("#modalEditarProducto .finOferta").val(respuesta[0]["expiredOffer"]);
      } else {
        ...
        $("#modalEditarProducto .previsualizarOferta").attr(
          "src",
          "vistas/img/ofertas/default/default.jpg"
        );
        $("#modalEditarProducto .antiguaFotoOferta").val(
          respuesta[0]["imgOffer"]
        );
      ...
      $("#modalEditarProducto .valorOferta").change(function () {
        if ($(this).attr("tipo") == "oferta") {
          var descuento =
            100 -
            (Number($(this).val()) * 100) /
              Number($("#modalEditarProducto .precio").val());
      ...
      $(".guardarCambiosProducto").click(function () {
        ...
        if (
          $("#modalEditarProducto .tituloProducto").val() != "" &&
          ...
          if (
            arrayFiles.length > 0 &&
            $("#modalEditarProducto .rutaProducto").val() != ""
          ) {
            var listaMultimedia = [];
            var finalFor = 0;
                ...
                success: function (respuesta) {
                  ...
                  if (localStorage.getItem("multimediaFisica") != null) {
                    var jsonLocalStorage = JSON.parse(
                      localStorage.getItem("multimediaFisica")
                    );

                    var jsonMultimediaFisica = listaMultimedia.concat(
                      jsonLocalStorage
                    );
...
function editarMiProducto(imagen) {
  ...
  var rutaProductoMercadoLibre = $(
    "#modalEditarProducto .rutaProductoMercadoLibre"
  ).val();
  var seleccionarTipo = $("#modalEditarProducto .seleccionarTipo").val();
  var seleccionarCategoria = $(
    "#modalEditarProducto .seleccionarCategoria"
  ).val();
  var seleccionarSubCategoria = $(
    "#modalEditarProducto .seleccionarSubCategoria"
  ).val();
  var descripcionProducto = $(
    "#modalEditarProducto .descripcionProducto"
  ).val();
  ...
  var detalles = {
    Marca: $("#modalEditarProducto .detalleMarca").val(),
    Codigo: $("#modalEditarProducto .codigoProducto").val(),
  };

  var detallesString = JSON.stringify(detalles);

  var antiguaFotoPortada = $("#modalEditarProducto .antiguaFotoPortada").val();
  var antiguaFotoPrincipal = $(
    "#modalEditarProducto .antiguaFotoPrincipal"
  ).val();
...
=============================================*/
$(".tablaFichasTecnicas tbody").on("click", ".btnEliminarFicha", function () {
  var idFicha = $(this).attr("idFicha");
  var url = $(this).attr("url");
  swal({
    title: "¿Está seguro de borrar la ficha tecnica?",
    text: "¡Si no lo está puede cancelar la accíón!",
    type: "warning",
    showCancelButton: true,
    confirmButtonColor: "#3085d6",
    ...
    if (result.value) {
      window.location =
        "index.php?ruta=fichasTecnicas&idFicha=" + idFicha + "&url=" + url;
    }
  });
});
