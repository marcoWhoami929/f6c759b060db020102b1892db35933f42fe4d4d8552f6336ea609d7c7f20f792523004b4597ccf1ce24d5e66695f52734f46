...
$(document).on(
  "change",
  ".seleccionarRutaBanner, .seleccionarTipoBanner",
  function () {
    $(".alert").remove();
      ...
      success: function (respuesta) {
        if (respuesta) {
          if (ruta == "sin-categoria") {
            $(".seleccionarTipoBanner")
              .parent()
              .after(
                '<div class="alert alert-warning">Esta ruta ya existe en la base de datos</div>'
              );

            $(".seleccionarTipoBanner").val("");
          } else {
            $(".seleccionarRutaBanner")
              .parent()
              .after(
                '<div class="alert alert-warning">Esta ruta ya existe en la base de datos</div>'
              );
...
$(".tablaBanner tbody").on("click", ".btnEditarBanner", function () {
  ...
  $.ajax({
    ...
    success: function (respuesta) {
      ...
      $("#modalEditarBanner .editarTituloBanner").val(respuesta["title"]);
      ...
      if (respuesta["tipo"] != "sin-categoria") {
        ...
        $("#modalEditarBanner .optionEditarRutaBanner").html(
          respuesta["route"]
        );

        $("#modalEditarBanner .seleccionarRutaBanner").attr(
          "name",
          "rutaBanner"
        );
...
$(".tablaBanner tbody").on("click", ".btnEliminarBanner", function () {
  ...
  swal({
    ...
    if (result.value) {
      window.location =
        "index.php?ruta=banner&idBanner=" +
        idBanner +
        "&imgBanner=" +
        imgBanner;
    }
