    ...
    success: function (respuesta) {
      ...
      if (respuesta.length != 0) {
        $(".validarSubCategoria")
          .parent()
          .after(
            '<div class="alert alert-warning">Esta Subcategoría ya existe en la base de datos</div>'
          );
...
$(".tablaSubCategorias tbody").on(
  ...
  function () {
    ...
    $.ajax({
      ...
      dataType: "json",
      success: function (respuesta) {
        $("#modalEditarSubCategoria .editarIdSubCategoria").val(
          respuesta[0]["id"]
        );
        $("#modalEditarSubCategoria .tituloSubCategoria").val(
        ...
        if (respuesta[0]["categoryId"] != 0) {
          ...
          $.ajax({
            ...
            dataType: "json",
            success: function (respuesta) {
              $("#modalEditarSubCategoria .seleccionarCategoria").val(
                respuesta["id"]
              );
              $("#modalEditarSubCategoria .optionEditarCategoria").html(
                respuesta["category"]
              );
            },
          });
        ...
        $.ajax({
          ...
          success: function (respuesta) {
            ...
            $("#modalEditarSubCategoria .editarIdCabecera").val(
              respuesta["id"]
            );
            ...
            $("#modalEditarSubCategoria .descripcionSubCategoria").val(
            ...
            if (respuesta["keywords"] != null) {
              $(".editarPalabrasClaves").html(
                '<div class="input-group">' +
                  ...
                  '<input type="text" class="form-control input-lg tagsInput pClavesSubCategoria" value="' +
                  respuesta["keywords"] +
                  '" data-role="tagsinput" name="pClavesSubCategoria">' +
                  "</div>"
              );

              $("#modalEditarSubCategoria .pClavesSubCategoria").tagsinput(
                "items"
              );
            ...
            $("#modalEditarSubCategoria .previsualizarPortada").attr(
              "src",
              respuesta["frontPage"]
            );
            $("#modalEditarSubCategoria .antiguaFotoPortada").val(
              respuesta["frontPage"]
            );
          },
        });
        ...
        if (respuesta[0]["offer"] != 0) {
          ...
          $("#modalEditarSubCategoria #precioOferta").val(
            respuesta[0]["offerPrice"]
          );
          $("#modalEditarSubCategoria #descuentoOferta").val(
            respuesta[0]["discountOffer"]
          );

          if (respuesta[0]["offerPrice"] != 0) {
            $("#modalEditarSubCategoria #precioOferta").prop("readonly", true);
            $("#modalEditarSubCategoria #descuentoOferta").prop(
              "readonly",
              false
            );
          }
          ...
          $("#modalEditarSubCategoria .previsualizarOferta").attr(
            "src",
            respuesta[0]["imgOffer"]
          );

          $("#modalEditarSubCategoria .antiguaFotoOferta").val(
            respuesta[0]["imgOffer"]
          );

          $("#modalEditarSubCategoria .finOferta").val(
            respuesta[0]["expiredOffer"]
          );
        } else {
...
$(".tablaSubCategorias").on("click", ".btnEliminarSubCategoria", function () {
  ...
  swal({
    ...
    if (result.value) {
      window.location =
        "index.php?ruta=subcategorias&idSubCategoria=" +
        idSubCategoria +
        "&imgOferta=" +
        imgOferta +
        "&rutaCabecera=" +
