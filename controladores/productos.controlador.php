...
class ControladorProductos
{
			...
			if (preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ ]+$/', $datos["tituloProducto"])) {
				...
				if ($datos["selActivarOferta"] == "oferta") {
					...
					$datosProducto = array(
						...
						"rutaMercadoLibre" => $datos["rutaProductoMercadoLibre"],
						"estado" => 1,
						"titular" => substr(trim($datos["descripcionProducto"]), 0, 225) . "...",
						"descripcion" => trim($datos["descripcionProducto"]),
				...
				ModeloCabeceras::mdlIngresarCabecera("headers", $datosProducto);

				$respuesta = ModeloProductos::mdlIngresarProducto("products", $datosProducto);
		...
		if (isset($datos["idProducto"])) {
			...
			if (preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ ]+$/', $datos["tituloProducto"])) {
				...
				if ($datos["tipo"] == "fisico") {
					...
					$traerProductos = ModeloProductos::mdlMostrarProductos("products", $item, $valor);
				...
				if (isset($datos["fotoPortada"]["tmp_name"]) && !empty($datos["fotoPortada"]["tmp_name"])) {
					...
					=============================================*/
					if ($datos["antiguaFotoPortada"] == 'vistas/img/cabeceras/default/default.png') {
					} else {
				...
				if (isset($datos["fotoPrincipal"]["tmp_name"]) && !empty($datos["fotoPrincipal"]["tmp_name"])) {
					...
					=============================================*/
					if ($datos["antiguaFotoPrincipal"] == 'vistas/img/productos/default/default.jpg') {
					} else {
				...
				ModeloCabeceras::mdlEditarCabecera("headers", $datosProducto);

				$respuesta = ModeloProductos::mdlEditarProducto("products", $datosProducto);
	...
	static public function ctrEliminarProducto()
	{
		...
		if (isset($_GET["idProducto"])) {
			...
			$respuesta = ModeloProductos::mdlEliminarProducto("products", $datos);
	...
	static public function ctrUploadFicha($datos)
	{

		$tabla = "technicasheets";

		$respuesta = ModeloProductos::mdlUploadFicha($tabla, $datos);
	...
	static public function ctrMostrarFichasTecnicas()
	{

		$tabla = "technicasheets";

		$respuesta = ModeloProductos::mdMostrarFichasTecnicas($tabla);
	...
	static public function ctrEliminarFichaTecnica()
	{
		...
		if (isset($_GET["idFicha"])) {
			...
			$respuesta = ModeloProductos::mdlEliminarFichaTecnica("technicasheets", $datos);
			...
			if ($respuesta == "ok") {
				...
				swal({
					  type: "success",
					  title: "La ficha ha sido eliminada correctamente",
					  showConfirmButton: true,
					  confirmButtonText: "Cerrar"
					  }).then(function(result){
								if (result.value) {

								window.location = "fichasTecnicas";
